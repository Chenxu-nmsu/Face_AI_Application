...
with tf.Session() as session:
    ...
    for step in range(100000):
        ...
        global_step_val,lr_val,  _, loss_val, l2_loss_val = session.run([global_step, lr, train_op, loss, l2_loss], 
                                                                feed_dict={input_x: batch_x, label: batch_y})
        print("ite:{}, loss:{}, l2:{}, lr_val:{}".format(step, loss_val, l2_loss_val, lr_val))

        summary_writer.add_summary(tf.Summary(value=[tf.Summary.Value(tag="loss1", 
                                                                        simple_value=loss_val)]),
                                                                        global_step=step)
        summary_writer.add_summary(tf.Summary(value=[tf.Summary.Value(tag="loss2", 
                                                                        simple_value=l2_loss_val)]),
                                                                        global_step=step)
        summary_writer.add_summary(tf.Summary(value=[tf.Summary.Value(tag="lr", 
                                                                        simple_value=lr_val)]),
                                                                        global_step=step)
        ...
        if step % 10000 == 0 and step > 0:
            saver.save(sess=session, save_path="models/senet.ckpt" + str(step))
