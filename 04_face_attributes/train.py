...
with tf.Session() as session:
    ...
    for step in range(1000000):
        batch_x, batch_eye, batch_male, batch_young, batch_smiling = session.run(
            [tf_im_batch, tf.label_eye_batch, tf.label_male_batch, tf.label_young_batch, tf.label_smiling_batch])
        
        _, loss_val, l2_loss_val, lr_val, global_step_val, loss_eyeglasses_val, \
        loss_male_val, loss_young_val, loss_smiling_val = session.run(
            [train_op, loss, l2_loss, lr, global_step, loss_eyeglasses, loss_male, loss_young, loss_smiling], 
            feed_dict = {input_x: batch_x, label_eyeglasses:batch_eye, 
                        label_male:batch_male, label_young:batch_young, label_smiling:batch_smiling})

        print("ite:{}, loss:{}, l2_loss:{}, loss_eye:{}, loss_male:{}, loss_young:{}, loss_smiling:{}, lr:{}".format(
            step, loss_val, l2_loss_val, loss_eyeglasses_val, loss_male_val, loss_young_val, loss_smiling_val, lr_val))

        summary_writer.add_summary(tf.Summary(
            value = [tf.Summary.Value(tag='loss1', simple_value=loss_val)]),
            global_step = step
        )

        summary_writer.add_summary(tf.Summary(
            value = [tf.Summary.Value(tag='loss2', simple_value=l2_loss_val)]),
            global_step = step
        )

        summary_writer.add_summary(tf.Summary(
            value = [tf.Summary.Value(tag='loss_eye', simple_value=loss_eyeglasses_val)]),
            global_step = step
        )

        summary_writer.add_summary(tf.Summary(
            value = [tf.Summary.Value(tag='loss_male', simple_value=loss_male_val)]),
            global_step = step
        )

        summary_writer.add_summary(tf.Summary(
            value = [tf.Summary.Value(tag='loss_young', simple_value=loss_young_val)]),
            global_step = step
        )

        summary_writer.add_summary(tf.Summary(
            value = [tf.Summary.Value(tag='loss_smile', simple_value=loss_smiling_val)]),
            global_step = step
        )

        summary_writer.add_summary(tf.Summary(
            value = [tf.Summary.Value(tag='lr', simple_value=lr_val)]),
            global_step = step
        )

        if step % 10000 == 0:
            saver.save(sess = session, save_path="models/model-{}.ckpt".format(step))

        if step % 500 = 0:
            batch_x, batch_eye, batch_male, batch_young, batch_smiling = session.run(
            [te_im_batch, te.label_eye_batch, te.label_male_batch, te.label_young_batch, te.label_smiling_batch])
        
            loss_val, l2_loss_val, loss_eyeglasses_val, loss_male_val, loss_young_val, loss_smiling_val = session.run(
                [loss, l2_loss, loss_eyeglasses, loss_male, loss_young, loss_smiling], 
                feed_dict = {input_x: batch_x, label_eyeglasses:batch_eye, 
                            label_male:batch_male, label_young:batch_young, label_smiling:batch_smiling})

            print("Test, ite:{}, loss:{}, l2_loss:{}, loss_eye:{}, loss_male:{}, loss_young:{}, loss_smiling:{}".format(
                step, loss_val, l2_loss_val, loss_eyeglasses_val, loss_male_val, loss_young_val, loss_smiling_val))
